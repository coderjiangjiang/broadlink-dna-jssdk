...
/*
    ...
    * */
const ACT_ADD = 0, ACT_DELETE = 1, ACT_EDIT = 2, ACT_LIST = 3, ACT_SWITCH = 4, ACT_LIMITATION = 5, ACT_SUN_SETTING = 6;
const TYPE_COMMON = 'comm', TYPE_DELAY = 'delay', TYPE_PERIOD = 'period', TYPE_CYCLE = 'cycle', TYPE_RAND = 'rand', TYPE_ALL = 'all', BEFORE_SUN_SET = 'D-', BEFORE_SUN_RISE = 'U-', AFTER_SUN_SET = 'D+', AFTER_SUN_RISE = 'U+';
const _simpleClone = (obj) => {
    ...
    const copy = Object.keys(obj).reduce((copy, field) => {
        ...
        if (Array.isArray(val)) {
            ...
            if (val && val.clone && (0, utils_js_1.isFunction)(val.clone)) {
                //如果对象本身提供了clone方法
                copyVal = val.clone();
            }
            else {
...
class Time {
    constructor(str) {
        if (str) {
            ...
            this.moment = (0, moment_1.default)().startOf('minute').add(1, 'minutes');
        }
    }
    isRepeated() {
    ...
    parse(str) {
        this.str = str;
        if (str.startsWith(BEFORE_SUN_SET) ||
            ...
            if (timeArr[6] === '*') {
                ...
                this.moment = (0, moment_1.default)(timeArr.slice(3).join('_'), 'DD_MM_*_YYYY');
            }
            this.duration = moment_1.default.duration({
            ...
            if (timeArr[6] === '*') {
                ...
                this.moment = (0, moment_1.default)(timeArr.slice(0, 3).join('_'), 'ss_mm_HH');
            }
            else {
                // year字段有值，单次定时
                //eg:0_1_22_26_2_*_2018
                this.moment = (0, moment_1.default)(str, 'ss_mm_HH_DD_MM_*_YYYY');
            }
        }
    }
    clone() {
...
class Timer {
    constructor(task) {
        if (typeof task === 'object') {
            ...
            this._override((cmd) => (0, utils_js_1.combine)(cmd), 'cmd', 'cmd1', 'cmd2')
                ._override((time) => new Time(time), 'time', 'stime', 'etime')
                ._override((en) => !!en, 'en')
                ._override((duration) => moment_1.default.duration(duration, 'seconds'), 'time1', 'time2');
        }
            ...
            if (TYPE_COMMON === task || TYPE_DELAY === task) {
                ...
                this.time1 = moment_1.default.duration(1, 'minutes');
                this.time2 = moment_1.default.duration(1, 'minutes');
            }
        }
    }
    setRepeat(repeat) {
    ...
    toOriginal() {
        ...
        override((cmd) => (0, utils_js_1.split)(cmd), 'cmd', 'cmd1', 'cmd2');
...
const call = (request) => __awaiter(void 0, void 0, void 0, function* () {
    const device = yield call_native_js_1.deviceInfoPromise;
    if (!('did' in request)) {
        request = Object.assign(Object.assign({}, request), { did: device.subDeviceID || device.deviceID });
    }
    const resp = yield (0, call_native_js_1.dnaControl)(request, 'dev_subdev_timer');
    return resp.data;
});
;
...
const list = function ({ type = TYPE_ALL, count = 10, index = 0, did, } = {}) {
...
const sunSetting = function (setting) {
    return call(Object.assign(Object.assign({}, setting), { act: ACT_SUN_SETTING }));
};
